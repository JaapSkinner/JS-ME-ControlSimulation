%% ANALYZE PREPROCESSED SIMULATION DATA (RLS Version) - FINAL CLEAN + TRIMMED
%
...
%
% COMPARISON: Estimated (RLS) vs. Nominal (Baseline Guess).
...
plotOutputPath = fullfile(parentFolder, 'RLS_Analysis_Plots_Trimmed'); 
...
% Ideally we shift them so they all start at 0 relative to the trim cut
...
try
    ...
    for k = 1:6
        ...
        if k == 1, legend('True', 'Estimated (RLS)', 'Nominal (Guess)', 'Location', 'northwest'); end 
    ...
    for k = 1:6
        ...
        title(dof_labels{k}); grid on;
        if k == 1, legend('Estimated (RLS) Error', 'Nominal (Guess) Error', 'Location', 'best'); end
        if k >= 5, xlabel('Time (s)'); end
        ylabel('Error');
    end
...
try
    ...
    for k=1:6
        ...
        title(dof_labels{k}); grid on;
        if k==1, legend('Estimated (RLS)', 'Nominal (Guess)'); end 
...
try
    ...
    set(h_est, {'linew'}, {1.5}); 
    title('Figure 3: Estimated (RLS) RMSE'); 
    ylabel('RMSE'); grid on;
...
try
    ...
    if isempty(s3)
        legend([s1 s2], 'Estimated (RLS)', 'Nominal (Guess)', 'Location', 'best');
    else
        legend([s1 s2 s3], 'Estimated (RLS)', 'Nominal (Guess)', 'Outliers', 'Location', 'best');
    end
...
try
    ...
    title('Fig 5: Estimated (RLS) Fingerprint'); 
