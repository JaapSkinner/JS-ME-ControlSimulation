% =========================================================================
% AUTOMATED SENSITIVITY TREND HUNTER (v2.3 - Efficient Logic)
% =========================================================================
% This script systematically analyzes all Monte Carlo results to find
% statistically significant linear relationships.
%
% VERSION 2.3 REFINEMENTS:
% - Moved the check for parameter variation into the main analysis loop.
%   This is more efficient and prevents meaningless correlation calculations.
%
clear; clc; close all;
...
for i = 1:numFiles
    ...
    for sp = 1:nSetpoints
        ...
        mega_table{row_idx, error_vars} = abs(error_raw');
        leakage = w_actual;
        leakage(w_des ~= 0) = 0;
        leakage_vars = {'Leakage_Fx','Leakage_Fy','Leakage_Fz','Leakage_Tx','Leakage_Ty','Leakage_Tz'};
        mega_table{row_idx, leakage_vars} = abs(leakage');
        row_idx = row_idx + 1;
    end
...
for sp = 1:nSetpoints
    ...
    for c = 1:length(causes)
        for e = 1:length(effects)
            ...
            % --- ADDED: Check for variation in the 'cause' parameter ---
            % If the parameter doesn't vary, correlation is meaningless.
            if range(x_data(valid_indices)) <= eps
                continue; 
            end
            % -----------------------------------------------------------
