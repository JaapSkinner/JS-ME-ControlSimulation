% =========================================================================
% GRANULAR SENSITIVITY ANALYSIS: CoM vs Wrench Components (Combined)
% =========================================================================
...
% --- MODIFIED: Hard-code the setpoints to analyze ---
...
% Pre-allocate the table, now twice as large to hold data from both setpoints
...
for i = 1:numFiles
    ...
    % Get the CoM deviation for this run (it's the same for both setpoints)
    cog_dev = [NaN, NaN, NaN]; % Default in case field is missing
    ...
    % Loop through the two setpoints we want to analyze for this file
    for sp = setpoints_to_analyze
        % Get data for the current setpoint
        w_des = data.setpoints(sp, :)';
        w_actual = data.wrenches(:, sp);
        error_raw = w_actual - w_des;
        
        % Populate the next row in the table
        ...
        results_table.Error_Fx(table_row_idx) = abs(error_raw(1));
        results_table.Error_Fy(table_row_idx) = abs(error_raw(2));
        results_table.Error_Fz(table_row_idx) = abs(error_raw(3));
        results_table.Error_Tx(table_row_idx) = abs(error_raw(4));
        results_table.Error_Ty(table_row_idx) = abs(error_raw(5));
        results_table.Error_Tz(table_row_idx) = abs(error_raw(6));
...
for c = 1:length(causes)
    for e = 1:length(effects)
        ...
        scatter(x_data, y_data, 15, 'b', 'filled', 'MarkerFaceAlpha', 0.2); % More transparency for more points
        hold on;
