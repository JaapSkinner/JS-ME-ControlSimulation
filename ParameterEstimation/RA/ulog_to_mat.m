%% ULog + Vicon CSV Merger & Sync (Octocopter Fixed + Euler/Quat Switch + Excitation)
...
raw_exc     = readTT('multisine_excitation_status'); % <--- ADDED EXCITATION TOPIC
...
% --- MULTISINE EXCITATION EXTRACTION ---
fprintf('Extracting Multisine Excitation...\n');
flightData.multisine_excitation_status = struct();
if ~isempty(raw_exc)
    exc_cols = cell(1, NUM_MOTORS);
    for m = 1:NUM_MOTORS
        exc_cols{m} = sprintf('excitation_%02d', m-1);
    end
    
    % Try extracting individual fields first (excitation_00, excitation_01)
    exc_data = extractScalars(raw_exc, exc_cols, t_master);
    
    % If empty, fallback to checking if it's logged as a single vector array
    if isempty(exc_data) || max(abs(exc_data(:))) == 0
        exc_data = extractVector(raw_exc, 'excitation', act_indices, t_master);
    end
    
    flightData.multisine_excitation_status.excitation = exc_data;
else
    fprintf('  > Topic multisine_excitation_status not found. Leaving empty.\n');
end
...
if hasMocap
    ...
    if ~isempty(col_frame) && ~isempty(col_tx)
        ...
        try
            ...
            elseif strcmp(MOCAP_ROTATION_TYPE, 'EULER')
                if size(raw_mat, 2) >= 8
                    rx = raw_mat(:, 3); ry = raw_mat(:, 4); rz = raw_mat(:, 5);
                    v_quat_neu = euler_to_q(rx, ry, rz);
    ...
    v_pos_neu = v_pos_neu(valid, :) / 1000.0; 
    if ~isempty(v_quat_neu); v_quat_neu = v_quat_neu(valid, :); end
    
    v_pos_ned = v_pos_neu;
    v_pos_ned(:, 2) = -v_pos_neu(:, 2); 
    v_pos_ned(:, 3) = -v_pos_neu(:, 3); 
